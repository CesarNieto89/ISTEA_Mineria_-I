...
def recomendar_libro (lista_libros, genero_recomendacion):
    libros_de_genero = [libro for libro in lista_libros if libro.genero == genero_recomendacion]
    if libros_de_genero:
        libro_recomendado = max (libros_de_genero, key=lambda libro: libro.puntuacion)  
        print (f"Se recomienda {libro_recomendado}")
    else:
        print(f"No hay libros para el género {libro_recomendado}")
